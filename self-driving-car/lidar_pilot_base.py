...
class LidarPilotBase:
    ...
    def sweep (self):   # Control algorithm to be tested
        ...
        if sp.driveManually == False:
            ...
            # scaled targetVelocity, (min = -1.0 and max = 1.0)
            self.scaledTargetVelocity = ((self.targetVelocity - (-1.5)) * (1.0 - (-1.0)) / (1.5 - (-1.5))) + (-1.0)
            self.kit.motor1.throttle = self.scaledTargetVelocity
